...
<p align="center">
  <a href="#功能特性">功能特性</a> •
  <a href="#为什么选择-onekeyclaw">为什么选择 OneKeyClaw</a> •
  <a href="#快速上手">快速上手</a> •
  <a href="#系统架构">系统架构</a> •
  <a href="#开发指南">开发指南</a> •
  <a href="#参与贡献">参与贡献</a>
...
**OneKeyClaw** 是连接强大 AI 智能体与普通用户之间的桥梁。基于 [OpenClaw](https://github.com/OpenClaw) 构建，它将命令行式的 AI 编排转变为易用、美观的桌面体验——无需使用终端。

无论是自动化工作流、连接通讯软件，还是调度智能定时任务，OneKeyClaw 都能提供高效易用的图形界面，帮助你充分发挥 AI 智能体的能力。

OneKeyClaw-Moyu 已预置魔芋供应商配置，原生支持 Windows 平台以及多语言设置。如需调整更细粒度的供应商细节，仍可通过 **设置 → 高级 → 开发者模式** 完成。
如果在开发或打包环境中设置 `VITE_PROVIDER_MODE=moyu`，引导页和模型配置页只会显示魔芋供应商，但仍保留完整的魔芋账户模型配置能力（`baseUrl`、`modelId`、多账户）。
...
构建 AI 智能体不应该需要精通命令行。OneKeyClaw 的设计理念很简单：**强大的技术值得拥有一个尊重用户时间的界面。**
...
| 进程管理繁琐 | 自动管理网关生命周期 |
| AI 供应商配置 | 内置魔芋供应商与引导式 API Key 配置 |
| 技能/插件安装复杂 | 内置技能市场与管理界面 |

### 内置 OpenClaw 核心

OneKeyClaw 直接基于官方 **OpenClaw** 核心构建。无需单独安装，我们将运行时嵌入应用内部，提供开箱即用的无缝体验。
...
当你使用 `@agent` 选择其他智能体时，OneKeyClaw 会直接切换到该智能体自己的对话上下文，而不是经过默认智能体转发。各 Agent 工作区默认彼此分离，但更强的运行时隔离仍取决于 OpenClaw 的 sandbox 配置。
...
OneKeyClaw 还会内置预装完整的文档处理技能（`pdf`、`xlsx`、`docx`、`pptx`），在启动时自动部署到托管技能目录（默认 `~/.openclaw/skills`），并在首次安装时默认启用。额外预装技能（`find-skills`、`self-improving-agent`、`tavily-search`、`brave-web-search`、`bocha-skill`）也会默认启用；若缺少必需的 API Key，OpenClaw 会在运行时给出配置错误提示。  
...
### 🔐 安全的供应商集成
OneKeyClaw-Moyu 默认集成魔芋供应商。API Key 会安全存储在系统原生密钥链中，如需高级供应商调优，仍可在开发者模式中处理。不设置 `VITE_PROVIDER_MODE=moyu` 时，前端会回到标准的多供应商配置界面。

### 🌙 自适应主题
支持浅色模式、深色模式或跟随系统主题。OneKeyClaw 自动适应你的偏好设置。

### 🚀 开机启动控制
在 **设置 → 通用** 中，你可以开启 **开机自动启动**，让 OneKeyClaw 在系统登录后自动启动。
...
首次启动 OneKeyClaw 时，**设置向导** 将引导你完成以下步骤：

1. **语言与区域** – 配置你的首选语言和地区
2. **AI 供应商** – 在引导流程中配置供应商，或通过 `VITE_PROVIDER_MODE=moyu` 将界面锁定为仅魔芋模式
3. **技能包** – 选择适用于常见场景的预配置技能
4. **验证** – 在进入主界面前测试你的配置

> Moonshot（Kimi）说明：OneKeyClaw 默认保持开启 Kimi 的 web search。  
> 当配置 Moonshot 后，OneKeyClaw 也会将 OpenClaw 配置中的 Kimi web search 同步到中国区端点（`https://api.moonshot.cn/v1`）。

### 代理设置

OneKeyClaw 内置了代理设置，适用于需要通过本地代理客户端访问外网的场景，包括 Electron 本身、OpenClaw Gateway，以及 Telegram 这类频道的联网请求。
...
- 如果启用了 Telegram，OneKeyClaw 还会把代理同步到 OpenClaw 的 Telegram 频道配置中。
...
OneKeyClaw 采用 **双进程 + Host API 统一接入架构**。渲染进程只调用统一客户端抽象，协议选择与进程生命周期由 Electron 主进程统一管理：
...
```bash
# 开发
pnpm run init             # 安装依赖并为当前平台准备内置二进制
pnpm dev                  # 以热重载模式启动
...
所有打包命令现在都会在运行 `electron-builder` 前自动准备内置二进制，因此产物会包含各平台所需的 `uv`，Windows 还会额外包含 `node.exe`。本地开发环境会优先使用适合国内网络的镜像源下载 `uv`/`node`，镜像不可用时会自动回退到官方源。
...
我们欢迎社区的各种贡献！无论是修复 Bug、开发新功能、改进文档还是翻译——每一份贡献都让 OneKeyClaw 变得更好。
...
我们正在启动 OneKeyClaw 合作伙伴计划，寻找能够帮助我们将 OneKeyClaw 介绍给更多客户的合作伙伴，尤其是那些有定制化 AI 智能体或自动化需求的客户。

合作伙伴负责帮助我们连接潜在用户和项目，OneKeyClaw 团队则提供完整的技术支持、定制开发与集成服务。
...
OneKeyClaw 基于 [MIT 许可证](LICENSE) 发布。你可以自由地使用、修改和分发本软件。
