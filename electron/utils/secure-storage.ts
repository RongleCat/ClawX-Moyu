 ...
 * Also synchronizes OneKeyClaw local provider list with OpenClaw's actual config.
 */
...
> {
  ...
  for (const provider of providers) {
    ...
    // we should remove it from OneKeyClaw UI to stay consistent.
    ...
    if (configMissing || (!isBuiltin && !isActive)) {
      console.log(`[Sync] Provider ${provider.id} (${provider.type}) missing from OpenClaw, dropping from OneKeyClaw UI`);
