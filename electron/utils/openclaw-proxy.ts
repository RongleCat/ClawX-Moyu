...
interface SyncProxyOptions {
   ...
   * currently disabled in OneKeyClaw settings.
   */
  preserveExistingWhenDisabled?: boolean;
}

/**
 * Sync OneKeyClaw global proxy settings into OpenClaw channel config where the
...
export async function syncProxyConfigToOpenClaw(
  ...
  return withConfigLock(async () => {
    ...
    if (!settings.proxyEnabled && preserveExistingWhenDisabled && currentProxy) {
      logger.info('Skipped Telegram proxy sync because OneKeyClaw proxy is disabled and preserve mode is enabled');
      return;
    }
