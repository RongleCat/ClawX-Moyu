...
# Post-installation script for OneKeyClaw on Linux
...
# Create symbolic link for OneKeyClaw app binary
if [ -x /opt/OneKeyClaw/onekeyclaw ]; then
    ln -sf /opt/OneKeyClaw/onekeyclaw /usr/local/bin/onekeyclaw 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/OneKeyClaw/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/OneKeyClaw/chrome-sandbox' || true
else
    chmod 0755 '/opt/OneKeyClaw/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/OneKeyClaw/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/onekeyclaw'
...
echo "OneKeyClaw has been installed successfully."
